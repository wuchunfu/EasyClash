      ...
      <div class="part">
        ...
        <div class="proxies">
          <div
            ...
            @dblclick="proxiders[proxies.now].type === 'select' ? changeProxy(proxiders[proxies.now].name, each.name) : cannotChangeProxyNotice(proxiders[proxies.now])"
          >
            <div class="cell-name">{{ each.name }}</div>
            <div class="cell-info">
              <div class="cell-type">{{ each.type }}</div>
...
const cannotChangeProxyNotice = (info: Record<string, any>) => {
  ElMessage.warning(`ProxyGroup ${info.name}  must type of select, now is ${info.type}`)
}
</script>
